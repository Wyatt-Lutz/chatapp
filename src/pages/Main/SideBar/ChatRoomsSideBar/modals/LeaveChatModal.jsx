...
const LeaveChatModal = ({setModal, contextMenuData, setContextMenu}) => {
  ...
  const onLeaveChat = async() => {
    setContextMenu({});
    setModal({type: ""});
    await removeUserFromChat(db, contextMenuData.chatID, currUser.uid, currUser.displayName, currUser.uid, contextMenuData.numOfMembers, chatDispatch, resetAllChatContexts);
  }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-6 bg-black/50">
      <div className="relative w-full max-w-md p-6 bg-gray-600 rounded-lg shadow-lg">
        <button onClick={onClose} className="absolute top-4 right-4"><CloseModal /></button>
