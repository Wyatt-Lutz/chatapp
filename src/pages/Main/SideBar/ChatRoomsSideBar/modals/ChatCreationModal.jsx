...
const ChatCreationModal = ({changeChatRoomCreationState}) => {
  ...
  const handleCreateChat = async () => {
    ...
    for (const member of usersToAdd) {
      membersList[member.userUid] = { isOnline: false, username: member.username, hasBeenRemoved: false, profilePictureURL: member.profilePictureURL };
    }
    ...
    const newChatID = await createChat(db, memberUids, title, tempTitle, membersList, uids, usersToAdd.length, currUserUid);
    console.log(newChatID);
    changeChatRoomCreationState(false);
    const updatedTempTitle = updateTempTitle(tempTitle, currUser.displayName);

    resetAllChatContexts();
    chatDispatch({ type: "CHANGE_CHAT", payload: { chatID: newChatID, title, owner: currUserUid, tempTitle: updatedTempTitle, numOfMembers: usersToAdd.length, firstMessageID: "" }});
  }
                ...
                <UserSearch addedUsers={addedUsers} setAddedUsers={setAddedUsers}/>
                  ...
                  {addedUsers.length > 2 && (
                    <div>
                      ...
                      <input maxLength={25} onChange={(e) => setChatTitleInputText(e.target.value)} value={chatTitleInputText}/>
                    </div>
                  )}
                  <div className="flex">
                    <button onClick={handleCreateChat} type="submit" disabled={addedUsers.length < 1} className="ring">Create Chat</button>
