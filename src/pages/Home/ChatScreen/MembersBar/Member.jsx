...
const Member = ({ memberUid, memberData, onContextMenu }) => {
  const { chatState } = useChatContexts();

  const memberLongPressHandlers = useLongPress((e) => {
    if (onContextMenu) {
      onContextMenu(e, memberUid, memberData);
    }
  });

  return (
    <div
      className="flex items-center justify-between px-3 py-2 bg-zinc-700/40 rounded-md hover:bg-zinc-700/30 transition"
      {...memberLongPressHandlers}
    >
