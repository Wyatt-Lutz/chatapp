  ...
  return (
    ...
    <div className="fixed inset-0 flex items-center justify-center p-6 bg-black/50">
      <div className="relative w-full max-w-md p-6 bg-gray-600 rounded-lg shadow-lg">
        <button onClick={() => setModal({ type: "", user: null })} className="absolute top-4 right-4"><CloseModal /></button>
      ...
      <div>
        <button className="border rounded-md bg-zinc-500 m-2 p-1" onClick={onAddUser}>Yes</button>
        <button className="border rounded-md bg-zinc-500 m-2 p-1" onClick={() => setModal({ type: "", user: null })}>No</button>
      </div>
